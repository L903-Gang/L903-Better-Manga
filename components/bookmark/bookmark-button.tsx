...
export default function BookmarkButton({ slug, name, image }: { slug: string; name: string; image: string }) {
  ...
  return (
    <TouchableOpacity style={[styles.button, { backgroundColor: bgColor }]} onPress={toggleBookmark} disabled={loading}>
      {loading ? (
        <ActivityIndicator color='#fff' />
      ) : (
        <Text style={styles.text}>
          {!userId ? 'Đăng nhập để bookmark' : bookmarked ? 'Xóa khỏi yêu thích' : 'Thêm vào yêu thích'}
        </Text>
      )}
    </TouchableOpacity>
  )
